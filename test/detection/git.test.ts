...
describe('c > detection > git > getGitRoot', () => {
...
describe('c > detection > git > isWorktree', () => {
  it('returns boolean', () => {
...
describe('c > detection > git > listWorktrees', () => {
  it('returns array', () => {
    const worktrees = listWorktrees();
    assert.ok(Array.isArray(worktrees));
  });

  it('returns at least one worktree (main repo)', () => {
    const worktrees = listWorktrees();
    // Main repo always appears in worktree list
    assert.ok(worktrees.length >= 1);
  });

  it('worktrees have path and branch', () => {
    const worktrees = listWorktrees();
